...
do
	...
	DispellFilter = dispellClasses[select(2, _G.UnitClass('player'))] or {}
end
...
local function OnUpdate(self, elapsed)
	...
	if (self.elapsed >= 0.1) then
		local timeLeft = self.endTime - _G.GetTime()
...
local function Update(self, event, unit)
	...
	local isCharmed = _G.UnitIsCharmed(unit)

	--store if we cand attack that unit, if its so the unit its hostile (Amber-Shaper Un'sok: Reshape Life)
	local canAttack = _G.UnitCanAttack("player", unit)

	for i = 1, 40 do
		local name, icon, count, debuffType, duration, expirationTime, unitCaster,
			isStealable, shouldConsolidate, spellId, canApplyAura, isBossDebuff = _G.UnitAura(unit, i, 'HARMFUL')
	...
	if self.RaidDebuffs.forceShow then
		_spellId = 6788
		_name, _, _icon = _G.GetSpellInfo(_spellId)
		_count, _dtype, _duration, _endTime, _stackThreshold = 5, 'Magic', 0, 60, 0
	end

	if _name then
		_stackThreshold = debuff_data[addon.MatchBySpellName and _name or _spellId] and
			debuff_data[addon.MatchBySpellName and _name or _spellId].stackThreshold or _stackThreshold
	end
